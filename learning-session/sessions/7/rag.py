from retriever import multi_query_retrieve
from memory import add_message, get_messages
from openai import OpenAI
from dotenv import load_dotenv
import os 

load_dotenv()

client = OpenAI(
    api_key=os.getenv("LLM_API_KEY"),
    base_url=(os.getenv("LLM_BASE_URL") or "").strip().strip("\"'"),
)


def ask_rag(question):
    add_message("user", question)

    user_department = os.getenv("USER_DEPARTMENT", "General")
    retrieved_chunks = multi_query_retrieve(
        question,
        user_department=user_department,
    )
    conversation_history = get_messages()
    if not retrieved_chunks:
        answer = "I don't have enough information."
        add_message("assistant", answer)
        return {
            "answer": answer,
            "citations": []
        }

    context = "\n\n".join([
        f"Source: {chunk['source']}\n{chunk['parent_text']}"
        for chunk in retrieved_chunks
    ])

    citations = [
        {
            "source": chunk["source"],
            "chunk": chunk["text"],
            "parent_context": chunk["parent_text"],
        }
        for chunk in retrieved_chunks
    ]

    response = client.chat.completions.create(
        model=(os.getenv("LLM_MODEL") or "").strip().strip("\"'"),
        messages=[
            {
                "role": "system",
                "content": f"""
Answer ONLY using provided context.
Use conversation history only to understand follow-up questions.

Context:
{context}

Conversation history:
{conversation_history}
"""
            },
            {
                "role": "user",
                "content": question
            }
        ]
    )

    answer = response.choices[0].message.content
    answer_with_citations = add_citations_to_answer(
        answer,
        citations
    )
    add_message("assistant", answer_with_citations)

    return {
        "answer": answer_with_citations,
        "citations": citations
    }


def add_citations_to_answer(answer, citations):
    citation_lines = []

    for index, citation in enumerate(citations, start=1):
        citation_lines.append(
            f"[{index}] Source: {citation['source']}\n"
            f"Passage: {citation['chunk']}"
        )

    return f"{answer}\n\nRetrieved passages:\n" + "\n\n".join(citation_lines)
